...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!ct.includes("audio")) {
      // Upstream returned HTML/JSON error page — fall through to alternate format
      const r2 = await fetch("https://text.pollinations.ai/openai", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!ct2.includes("audio")) return res.status(502).json({ error: "Speech generation failed: no audio returned" });
