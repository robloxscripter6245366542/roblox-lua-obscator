...
-- ── Palette (Creamy warm dark — parchment, gold, warm brown) ─────────────────────
local C = {
    BG     = Color3.fromRGB(24, 20, 16),    -- very dark warm brown
    SIDE   = Color3.fromRGB(16, 13, 10),    -- deeper warm brown
    PANEL  = Color3.fromRGB(34, 28, 22),    -- warm panel
    CARD   = Color3.fromRGB(46, 38, 30),    -- warm card
    DARK   = Color3.fromRGB(14, 11,  8),    -- near-black warm
    BORDER = Color3.fromRGB(70, 58, 44),    -- warm border
    ACCENT = Color3.fromRGB(210,168, 95),   -- warm gold
    ACC2   = Color3.fromRGB(240,205,135),   -- light cream gold
    GREEN  = Color3.fromRGB(82, 196, 120),
    RED    = Color3.fromRGB(235,  75,  75),
    YELLOW = Color3.fromRGB(250, 190,  40),
    PINK   = Color3.fromRGB(230,  90, 150),
    WHITE  = Color3.fromRGB(245, 240, 228),  -- cream white
    TEXT   = Color3.fromRGB(210, 198, 180),  -- warm text
    MUTED  = Color3.fromRGB(130, 115,  95),  -- warm muted
    -- Claude branding
    CLAY   = Color3.fromRGB(195, 120,  55),
    CREAM  = Color3.fromRGB(248, 242, 228),
}
...
print("[CHub] Kill Aura — press K to toggle, walks into range")
        ...
        -- ═══ GAME HUBS (30 games, no require, self-contained) ═══════════════════
        {name="Arsenal Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}
local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs)
    local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end
    local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true
    local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0)
    w.BackgroundColor3=Color3.fromRGB(20,22,34); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12)
    local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0)
    tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.new(1,1,1); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left
    local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5)
    xb.BackgroundColor3=Color3.fromRGB(244,63,94); xb.Text="✕"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6)
    xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end)
    local st={}
    for i,e in ipairs(fs) do
        local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28)
        b.BackgroundColor3=Color3.fromRGB(36,41,60); b.Text=e[1]; b.TextColor3=Color3.fromRGB(190,198,222); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8)
        st[i]=false; local ci=i
        b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(74,152,245) or Color3.fromRGB(36,41,60); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(190,198,222); pcall(e[2],st[ci]) end)
    end
    local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end)
    w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end)
end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local aimC; local function aim(on) if aimC then aimC:Disconnect();aimC=nil end; if not on then return end
    aimC=ac(rs.RenderStepped:Connect(function() if not uis:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then return end
        local best,bd=nil,math.huge; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local h=p.Character:FindFirstChild("Head"); if h then local pos,vis=cam:WorldToScreenPoint(h.Position); if vis then local d=(Vector2.new(pos.X,pos.Y)-Vector2.new(cam.ViewportSize.X/2,cam.ViewportSize.Y/2)).Magnitude; if d<bd then best=h;bd=d end end end end end
        if best then cam.CFrame=CFrame.lookAt(cam.CFrame.Position,best.Position) end end)) end
local espT={}; local function esp(on) for _,t in pairs(espT) do pcall(function()t:Destroy()end) end;espT={}; if not on then return end
    local function tag(p) if p==lp then return end; local function at() local c=p.Character;if not c then return end; local h=c:FindFirstChild("HumanoidRootPart");if not h then return end
        local bb=Instance.new("BillboardGui",h);bb.Size=UDim2.new(0,0,0,40);bb.StudsOffset=Vector3.new(0,3.2,0);bb.AlwaysOnTop=true
        local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,0,22);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=Color3.fromRGB(255,80,80);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=14
        local dl=Instance.new("TextLabel",bb);dl.Size=UDim2.new(1,0,0,16);dl.Position=UDim2.new(0,0,0,24);dl.BackgroundTransparency=1;dl.TextColor3=Color3.fromRGB(220,220,220);dl.TextStrokeTransparency=0;dl.Font=Enum.Font.Gotham;dl.TextSize=11
        ac(rs.Heartbeat:Connect(function() local mc=lp.Character and lp.Character:FindFirstChild("HumanoidRootPart"); dl.Text=mc and math.floor((h.Position-mc.Position).Magnitude).."m" or "" end)); espT[p]=bb end
        at(); ac(p.CharacterAdded:Connect(at)) end
    for _,p in pairs(pl:GetPlayers()) do tag(p) end; ac(pl.PlayerAdded:Connect(tag)) end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end
    if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100
        ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*80;flyBG.CFrame=cam.CFrame end)) end end
local ncC; local function nc(on) if ncC then ncC:Disconnect();ncC=nil end; if on then ncC=ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end
hub("⚔ Arsenal Hub",{
    {"Aimbot (hold RMB)",aim},{"Player ESP",esp},
    {"Speed+ (28 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 28 or 16 end end},
    {"Noclip",nc},{"Fly (WASD+Space)",fly},
    {"Infinite Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Silent Aim",function(on)
        if on and hookmetamethod and getnamecallmethod then
            local old; old=hookmetamethod(game,"__namecall",function(self,...) if getnamecallmethod()=="FindPartOnRayWithIgnoreList" then local r=pl:GetPlayers(); for _,p in pairs(r) do if p~=lp and p.Character then local h=p.Character:FindFirstChild("Head"); if h then local s=select(1,...); return old(self,RaycastParams.new(),h.Position) end end end end; return old(self,...) end) end end},
})
]]},
        {name="Rivals Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs)
    local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end
    local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true
    local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0)
    w.BackgroundColor3=Color3.fromRGB(20,22,34); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12)
    local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.new(1,1,1); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left
    local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(244,63,94); xb.Text="✕"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6)
    xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end)
    local st={} for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(36,41,60); b.Text=e[1]; b.TextColor3=Color3.fromRGB(190,198,222); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(74,152,245) or Color3.fromRGB(36,41,60); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(190,198,222); pcall(e[2],st[ci]) end) end
    local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end)
end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local aimC; local function aim(on) if aimC then aimC:Disconnect();aimC=nil end; if not on then return end; aimC=ac(rs.RenderStepped:Connect(function() if not uis:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then return end; local best,bd=nil,math.huge; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local h=p.Character:FindFirstChild("Head"); if h then local pos,vis=cam:WorldToScreenPoint(h.Position); if vis then local d=(Vector2.new(pos.X,pos.Y)-Vector2.new(cam.ViewportSize.X/2,cam.ViewportSize.Y/2)).Magnitude; if d<bd then best=h;bd=d end end end end end; if best then cam.CFrame=CFrame.lookAt(cam.CFrame.Position,best.Position) end end)) end
local espT={}; local function esp(on) for _,t in pairs(espT) do pcall(function()t:Destroy()end) end;espT={}; if not on then return end; local function tag(p) if p==lp then return end; local function at() local c=p.Character;if not c then return end; local h=c:FindFirstChild("HumanoidRootPart");if not h then return end; local bb=Instance.new("BillboardGui",h);bb.Size=UDim2.new(0,0,0,40);bb.StudsOffset=Vector3.new(0,3.2,0);bb.AlwaysOnTop=true; local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,0,22);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=Color3.fromRGB(255,80,80);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=14; local dl=Instance.new("TextLabel",bb);dl.Size=UDim2.new(1,0,0,16);dl.Position=UDim2.new(0,0,0,24);dl.BackgroundTransparency=1;dl.TextColor3=Color3.fromRGB(220,220,220);dl.TextStrokeTransparency=0;dl.Font=Enum.Font.Gotham;dl.TextSize=11; ac(rs.Heartbeat:Connect(function() local mc=lp.Character and lp.Character:FindFirstChild("HumanoidRootPart"); dl.Text=mc and math.floor((h.Position-mc.Position).Magnitude).."m" or "" end)); espT[p]=bb end; at(); ac(p.CharacterAdded:Connect(at)) end; for _,p in pairs(pl:GetPlayers()) do tag(p) end; ac(pl.PlayerAdded:Connect(tag)) end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*80;flyBG.CFrame=cam.CFrame end)) end end
local ncC; local function nc(on) if ncC then ncC:Disconnect();ncC=nil end; if on then ncC=ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end
hub("🎯 Rivals Hub",{
    {"Aimbot (hold RMB)",aim},{"Player ESP",esp},
    {"Speed+ (32 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 32 or 16 end end},
    {"Fly",fly},{"Noclip",nc},
    {"Hitbox+ (6 studs)",function(on) for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local h=p.Character:FindFirstChild("HumanoidRootPart"); if h then h.Size=on and Vector3.new(6,6,6) or Vector3.new(2,2,1) end end end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="Brookhaven Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs)
    local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(20,22,34); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.new(1,1,1); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(244,63,94); xb.Text="✕"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(36,41,60); b.Text=e[1]; b.TextColor3=Color3.fromRGB(190,198,222); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(74,152,245) or Color3.fromRGB(36,41,60); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(190,198,222); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end)
end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*80;flyBG.CFrame=cam.CFrame end)) end end
local ncC; local function nc(on) if ncC then ncC:Disconnect();ncC=nil end; if on then ncC=ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end
hub("🏠 Brookhaven Hub",{
    {"Speed+ (24 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 24 or 16 end end},
    {"Fly (WASD+Space)",fly},{"Noclip",nc},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Invisible",function(on) local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") or p:IsA("Decal") then p.Transparency=on and 1 or 0 end end end end},
    {"Fullbright",function(on) local l=game:GetService("Lighting"); l.Brightness=on and 2 or 1; l.ClockTime=on and 14 or 14; l.GlobalShadows=not on end},
    {"TP to Spawn",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(0,5,0) end end},
    {"Remove All Cars",function() for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Model") and v.Name:find("Car") then pcall(function()v:Destroy()end) end end end},
})
]]},
        {name="Blox Fruits Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(20,22,34); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.new(1,1,1); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(244,63,94); xb.Text="✕"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(36,41,60); b.Text=e[1]; b.TextColor3=Color3.fromRGB(190,198,222); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(74,152,245) or Color3.fromRGB(36,41,60); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(190,198,222); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*120;flyBG.CFrame=cam.CFrame end)) end end
local autoFruitC; local function autoFruit(on)
    if autoFruitC then autoFruitC:Disconnect();autoFruitC=nil end
    if not on then return end
    autoFruitC=ac(rs.Heartbeat:Connect(function()
        local hrp=getHRP(); if not hrp then return end
        for _,v in pairs(workspace:GetDescendants()) do
            if v:IsA("Model") and (v.Name:find("Fruit") or v.Name:find("fruit")) then
                local p=v:FindFirstChild("Handle") or v.PrimaryPart or v:FindFirstChildWhichIsA("BasePart")
...
hub("🍎 Blox Fruits Hub",{
    {"Speed+ (40 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 40 or 22 end end},
    {"Fly Fast (WASD)",fly},
    {"Inf Stamina",function(on) ac(rs.Heartbeat:Connect(function() if not on then return end; local c=lp.Character; local e=c and c:FindFirstChild("Energy"); if e then e.Value=math.huge end end)) end},
    {"Auto Collect Fruits",autoFruit},
    {"Fruit ESP",function(on) for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Model") and v.Name:find("Fruit") then local p=v:FindFirstChildWhichIsA("BasePart"); if p then local bb=Instance.new("BillboardGui",p);bb.AlwaysOnTop=true;bb.Size=UDim2.new(0,0,0,30);bb.StudsOffset=Vector3.new(0,2,0); local l=Instance.new("TextLabel",bb);l.Size=UDim2.new(1,0,1,0);l.BackgroundTransparency=1;l.Text="🍎 "..v.Name;l.TextColor3=Color3.fromRGB(255,200,80);l.TextStrokeTransparency=0;l.Font=Enum.Font.GothamBold;l.TextSize=12 end end end end},
    {"No Clip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="Da Hood Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(20,22,34); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.new(1,1,1); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(244,63,94); xb.Text="✕"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(36,41,60); b.Text=e[1]; b.TextColor3=Color3.fromRGB(190,198,222); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(74,152,245) or Color3.fromRGB(36,41,60); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(190,198,222); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("🔫 Da Hood Hub",{
    {"Aimbot (hold RMB)",aim},{"Player ESP",esp},
    {"Speed+ (40 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 40 or 16 end end},
    {"Hitbox+ (8 studs)",function(on) for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local h=p.Character:FindFirstChild("HumanoidRootPart"); if h then h.Size=on and Vector3.new(8,8,8) or Vector3.new(2,2,1); h.Transparency=on and 0.8 or 0 end end end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"Godmode",function(on) local function applyGod(c) local h=c:WaitForChild("Humanoid",5);if h then h.MaxHealth=math.huge;h.Health=math.huge end end; if lp.Character then applyGod(lp.Character) end; ac(lp.CharacterAdded:Connect(applyGod)) end},
    {"Kill Aura (8 stud)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP();if not hrp then return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local ph=p.Character:FindFirstChild("HumanoidRootPart"); if ph and (hrp.Position-ph.Position).Magnitude<8 then local hm=p.Character:FindFirstChildOfClass("Humanoid");if hm then hm.Health=0 end end end end end)) end end},
})
]]},
        {name="Murder Mystery 2 Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(20,22,34); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.new(1,1,1); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(244,63,94); xb.Text="✕"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(36,41,60); b.Text=e[1]; b.TextColor3=Color3.fromRGB(190,198,222); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(74,152,245) or Color3.fromRGB(36,41,60); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(190,198,222); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
hub("🔪 MM2 Hub",{
    {"Role ESP (Sheriff=blue/Murderer=red)",function(on)
        for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then
            ...
            for _,t in pairs(p.Character:GetChildren()) do
                if t:IsA("Tool") then
                    if t.Name:lower():find("knife") or t.Name:lower():find("murder") then col=Color3.fromRGB(255,60,60)
                    elseif t.Name:lower():find("gun") or t.Name:lower():find("sheriff") then col=Color3.fromRGB(60,120,255) end
                end
            end
            local bb=Instance.new("BillboardGui",hrp);bb.Size=UDim2.new(0,0,0,32);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true
            local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=col;nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=13
        end end
    end},
    {"Coin ESP",function(on) for _,v in pairs(workspace:GetDescendants()) do if v.Name=="Coin" or v.Name=="coin" then local bb=Instance.new("BillboardGui",v);bb.Size=UDim2.new(0,0,0,24);bb.StudsOffset=Vector3.new(0,2,0);bb.AlwaysOnTop=true; local l=Instance.new("TextLabel",bb);l.Size=UDim2.new(1,0,1,0);l.BackgroundTransparency=1;l.Text="💰";l.TextColor3=Color3.fromRGB(255,215,0);l.TextSize=18 end end end},
    {"Speed+ (24 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 24 or 16 end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Godmode",function(on) local function ag(c) local h=c:WaitForChild("Humanoid",5);if h then h.MaxHealth=math.huge;h.Health=math.huge end end; if lp.Character then ag(lp.Character) end; ac(lp.CharacterAdded:Connect(ag)) end},
})
]]},
        {name="Jailbreak Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(20,22,34); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.new(1,1,1); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(244,63,94); xb.Text="✕"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(36,41,60); b.Text=e[1]; b.TextColor3=Color3.fromRGB(190,198,222); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(74,152,245) or Color3.fromRGB(36,41,60); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(190,198,222); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*80;flyBG.CFrame=cam.CFrame end)) end end
hub("🚔 Jailbreak Hub",{
    {"Speed+ (50 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 50 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"TP Bank (0,5,0)",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(172,18,-907) end end},
    {"TP Police HQ",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(-54,0.5,487) end end},
    {"ESP Players",function(on) local espT={}; for _,t in pairs(espT) do pcall(function()t:Destroy()end) end;espT={}; if not on then return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local hrp=p.Character:FindFirstChild("HumanoidRootPart"); if hrp then local bb=Instance.new("BillboardGui",hrp);bb.Size=UDim2.new(0,0,0,32);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true; local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=Color3.fromRGB(255,80,80);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=13;espT[p]=bb end end end end},
})
]]},
        {name="Tower of Hell Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*80;flyBG.CFrame=cam.CFrame end)) end end
hub("Tower of Hell Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Skip Stage (TP Top)",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(hrp.Position+Vector3.new(0,600,0)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"Anti-Void",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();local hrp=getHRP(); if hrp and hrp.Position.Y<-80 and h then h.Health=0 end end)) end end},
    {"Bright Map",function(on) for _,p in pairs(workspace:GetDescendants()) do if p:IsA("BasePart") then p.Material=on and Enum.Material.Neon or Enum.Material.SmoothPlastic end end end},
})
]]},
        {name="Adopt Me Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*60;flyBG.CFrame=cam.CFrame end)) end end
hub("Adopt Me Hub",{
    {"Speed+ (50 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 50 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Auto Collect Nearby",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and v.CanTouch and (v.Name:lower():find("coin") or v.Name:lower():find("gift") or v.Name:lower():find("buck")) then if (v.Position-hrp.Position).Magnitude<20 then hrp.CFrame=CFrame.new(v.Position) end end end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"ESP Players",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__esp__" then v:Destroy() end end; return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local hrp=p.Character:FindFirstChild("HumanoidRootPart"); if hrp then local bb=Instance.new("BillboardGui",hrp);bb.Name="__esp__";bb.Size=UDim2.new(0,0,0,28);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true; local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=Color3.fromRGB(255,210,80);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=12 end end end end},
    {"TP Nursery",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(-33,0.5,-105) end end},
})
]]},
        {name="Pet Simulator X Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*70;flyBG.CFrame=cam.CFrame end)) end end
hub("Pet Sim X Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Auto Farm (Chase Coins)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; local near,nd=nil,60; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name=="Coin" or v.Name=="Gem" or v.Name:lower():find("orb") or v.Name:lower():find("coin")) then local d=(v.Position-hrp.Position).Magnitude; if d<nd then near,nd=v,d end end end; if near then hrp.CFrame=CFrame.new(near.Position+Vector3.new(0,3,0)) end end)) end end},
    {"Auto Break Eggs",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and v.Name:lower():find("egg") then local d=(v.Position-hrp.Position).Magnitude; if d<4 then hrp.CFrame=CFrame.new(v.Position) elseif d<60 then hrp.CFrame=CFrame.new(hrp.Position:Lerp(v.Position,0.1)) end end end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"ESP Eggs",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__pesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and v.Name:lower():find("egg") then local bb=Instance.new("BillboardGui",v);bb.Name="__pesp__";bb.Size=UDim2.new(0,0,0,24);bb.StudsOffset=Vector3.new(0,2,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=v.Name;nl.TextColor3=Color3.fromRGB(120,255,120);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=11 end end end},
})
]]},
        {name="BedWars Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*80;flyBG.CFrame=cam.CFrame end)) end end
hub("BedWars Hub",{
    {"Speed+ (55 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 55 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Kill Aura (20st)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local h2=p.Character:FindFirstChildOfClass("Humanoid"); local r2=p.Character:FindFirstChild("HumanoidRootPart"); if h2 and r2 and (r2.Position-hrp.Position).Magnitude<20 then pcall(function() h2.Health=0 end) end end end end)) end end},
    {"ESP Players",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__bwesp__" then v:Destroy() end end; return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local hrp=p.Character:FindFirstChild("HumanoidRootPart"); if hrp then local bb=Instance.new("BillboardGui",hrp);bb.Name="__bwesp__";bb.Size=UDim2.new(0,0,0,28);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=Color3.fromRGB(255,100,100);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=12 end end end end},
    {"Aimbot (RMB hold)",function(on) if on then ac(rs.RenderStepped:Connect(function() if not uis:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then return end; local near,nd=nil,300; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local r2=p.Character:FindFirstChild("HumanoidRootPart"); if r2 then local d=(r2.Position-cam.CFrame.Position).Magnitude; if d<nd then near,nd=r2,d end end end end; if near then cam.CFrame=CFrame.lookAt(cam.CFrame.Position,near.Position+Vector3.new(0,1.5,0)) end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
})
]]},
        {name="Doors Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("Doors Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Noclip (through walls)",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"God Mode",function(on) local h=getHum(); if h then if on then h.MaxHealth=1e9;h.Health=1e9; ac(rs.Heartbeat:Connect(function() local hh=getHum();if hh then hh.Health=1e9 end end)) end end end},
    {"ESP Entities",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__desp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Model") and (v.Name=="Figure" or v.Name=="Seek" or v.Name=="Rush" or v.Name=="Ambush" or v.Name=="Eyes" or v.Name=="Screech" or v.Name=="Halt" or v.Name=="Timothy") then local hrp=v:FindFirstChild("HumanoidRootPart") or v:FindFirstChildOfClass("BasePart"); if hrp then local bb=Instance.new("BillboardGui",hrp);bb.Name="__desp__";bb.Size=UDim2.new(0,0,0,32);bb.StudsOffset=Vector3.new(0,4,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text="[!] "..v.Name;nl.TextColor3=Color3.fromRGB(255,60,60);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=13 end end end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Hide from Entities",function(on) local hrp=getHRP(); if hrp then hrp.Anchored=on end end},
    {"Bright (Fullbright)",function(on) game:GetService("Lighting").Brightness=on and 2 or 1; game:GetService("Lighting").ClockTime=on and 14 or game:GetService("Lighting").ClockTime end},
})
]]},
        {name="Natural Disaster Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*80;flyBG.CFrame=cam.CFrame end)) end end
hub("Natural Disaster Hub",{
    {"Speed+ (70 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 70 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"TP Safe Spot (High)",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(hrp.Position.X,300,hrp.Position.Z) end end},
    {"Remove All Hazards",function() for _,v in pairs(workspace:GetDescendants()) do pcall(function() if v:IsA("BasePart") and (v.Name:lower():find("lava") or v.Name:lower():find("fire") or v.Name:lower():find("flood") or v.Name:lower():find("meteor")) then v:Destroy() end end) end end},
})
]]},
        {name="Royale High Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("Royale High Hub",{
    ...
    {"Fly (WASD+Space)",fly},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Auto Diamond Farm",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; local near,nd=nil,80; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("diamond") or v.Name:lower():find("diamond")) then local d=(v.Position-hrp.Position).Magnitude; if d<nd then near,nd=v,d end end end; if near then hrp.CFrame=CFrame.new(near.Position+Vector3.new(0,2,0)) end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"Fullbright",function(on) local L=game:GetService("Lighting"); L.Brightness=on and 2 or 1; L.ClockTime=on and 14 or 14 end},
    {"No Gravity",function(on) workspace.Gravity=on and 10 or 196.2 end},
})
]]},
        {name="Combat Warriors Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*80;flyBG.CFrame=cam.CFrame end)) end end
hub("Combat Warriors Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Kill Aura (15st)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local h2=p.Character:FindFirstChildOfClass("Humanoid"); local r2=p.Character:FindFirstChild("HumanoidRootPart"); if h2 and r2 and (r2.Position-hrp.Position).Magnitude<15 then pcall(function() h2.Health=0 end) end end end end)) end end},
    {"Aimbot (RMB)",function(on) if on then ac(rs.RenderStepped:Connect(function() if not uis:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then return end; local near,nd=nil,300; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local r2=p.Character:FindFirstChild("HumanoidRootPart"); if r2 then local d=(r2.Position-cam.CFrame.Position).Magnitude; if d<nd then near,nd=r2,d end end end end; if near then cam.CFrame=CFrame.lookAt(cam.CFrame.Position,near.Position+Vector3.new(0,1.5,0)) end end)) end end},
    {"ESP Players",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__cwesp__" then v:Destroy() end end; return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local hrp=p.Character:FindFirstChild("HumanoidRootPart"); if hrp then local bb=Instance.new("BillboardGui",hrp);bb.Name="__cwesp__";bb.Size=UDim2.new(0,0,0,28);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=Color3.fromRGB(255,80,80);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=12 end end end end},
    {"Hitbox Expand",function(on) for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then for _,v in pairs(p.Character:GetDescendants()) do if v:IsA("BasePart") then v.Size=on and Vector3.new(8,8,8) or Vector3.new(2,2,2) end end end end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="Work at Pizza Place Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*60;flyBG.CFrame=cam.CFrame end)) end end
hub("Work at Pizza Place Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Auto Work (Chase Tasks)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; local near,nd=nil,80; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("order") or v.Name:lower():find("pizza") or v.Name:lower():find("dough") or v.Name:lower():find("sauce") or v.Name:lower():find("box")) then local d=(v.Position-hrp.Position).Magnitude; if d<nd then near,nd=v,d end end end; if near then hrp.CFrame=CFrame.new(near.Position+Vector3.new(0,3,0)) end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"ESP Players",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__pzesp__" then v:Destroy() end end; return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local hrp=p.Character:FindFirstChild("HumanoidRootPart"); if hrp then local bb=Instance.new("BillboardGui",hrp);bb.Name="__pzesp__";bb.Size=UDim2.new(0,0,0,24);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=Color3.fromRGB(255,200,80);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=12 end end end end},
})
]]},
        {name="Build a Boat Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*60;flyBG.CFrame=cam.CFrame end)) end end
hub("Build a Boat Hub",{
    {"Speed+ (50 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 50 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Auto Collect Gold",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("gold") or v.Name:lower():find("crate") or v.Name:lower():find("chest")) then if (v.Position-hrp.Position).Magnitude<4 then hrp.CFrame=CFrame.new(v.Position+Vector3.new(0,3,0)) elseif (v.Position-hrp.Position).Magnitude<60 then hrp.CFrame=CFrame.new(hrp.Position:Lerp(v.Position,0.08)) end end end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"TP to Sea End",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(hrp.Position.X+5000,hrp.Position.Y,hrp.Position.Z) end end},
    {"No Water Drown",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
})
]]},
        {name="Car Crushers 2 Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*70;flyBG.CFrame=cam.CFrame end)) end end
hub("Car Crushers 2 Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"TP Nuclear Plant",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(0,10,0) end end},
    {"Auto Crusher Farm",function(on) if on then task.spawn(function() while on do local hrp=getHRP(); if hrp then local near,nd=nil,200; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("crusher") or v.Name:lower():find("conveyor")) then local d=(v.Position-hrp.Position).Magnitude; if d<nd then near,nd=v,d end end end; if near then hrp.CFrame=CFrame.new(near.Position+Vector3.new(0,5,0)) end end; task.wait(1.5) end end) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="Funky Friday Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
hub("Funky Friday Hub",{
    {"Auto Hit Notes",function(on) if on then ac(rs.Heartbeat:Connect(function() local pg=lp:FindFirstChild("PlayerGui"); if not pg then return end; for _,gui in pairs(pg:GetDescendants()) do if gui:IsA("ImageButton") or gui:IsA("TextButton") then if gui.Visible and gui.AbsoluteSize.X<80 then pcall(function() gui:SimulateClick() end) pcall(function() local vui=game:GetService("VirtualUser"); vui:Button1Down(gui.AbsolutePosition+gui.AbsoluteSize*0.5,workspace.CurrentCamera.CFrame); task.wait(); vui:Button1Up(gui.AbsolutePosition+gui.AbsoluteSize*0.5,workspace.CurrentCamera.CFrame) end) end end end end)) end end},
    {"Speed+ (50 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 50 or 16 end end},
    {"Inf Jump",function(on) if on then ac(game:GetService("UserInputService").JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Score Multiplier x2",function(on) if on then ac(rs.Heartbeat:Connect(function() local pg=lp:FindFirstChild("PlayerGui"); if pg then for _,v in pairs(pg:GetDescendants()) do if v:IsA("TextLabel") and tonumber(v.Text) then pcall(function() v.Text=tostring(tonumber(v.Text)*2) end) end end end end)) end end},
})
]]},
        {name="Shindo Life Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*90;flyBG.CFrame=cam.CFrame end)) end end
hub("Shindo Life Hub",{
    {"Speed+ (80 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 80 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Kill Aura (25st)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 and (r2.Position-hrp.Position).Magnitude<25 then pcall(function() v.Health=0 end) end end end end)) end end},
    {"Auto Farm (Chase NPC)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; local near,nd=nil,100; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 then local d=(r2.Position-hrp.Position).Magnitude; if d<nd then near,nd=r2,d end end end end; if near then hrp.CFrame=CFrame.new(hrp.Position:Lerp(near.Position+Vector3.new(0,2,0),0.15)) end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"ESP NPCs",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__slesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Parent~=lp.Character and v.Parent:FindFirstChild("HumanoidRootPart") then local hrp=v.Parent.HumanoidRootPart; local bb=Instance.new("BillboardGui",hrp);bb.Name="__slesp__";bb.Size=UDim2.new(0,0,0,26);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=v.Parent.Name.." ["..math.floor(v.Health).."]";nl.TextColor3=Color3.fromRGB(255,180,50);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=11 end end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
})
]]},
        {name="Demonfall Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("Demonfall Hub",{
    {"Speed+ (80 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 80 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Kill Aura (20st)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 and (r2.Position-hrp.Position).Magnitude<20 then pcall(function() v.Health=0 end) end end end end)) end end},
    {"Auto Demon Farm",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; local near,nd=nil,120; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 then local d=(r2.Position-hrp.Position).Magnitude; if d<nd then near,nd=r2,d end end end end; if near then hrp.CFrame=CFrame.new(hrp.Position:Lerp(near.Position+Vector3.new(0,3,0),0.12)) end end)) end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"ESP Demons",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__dfesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Parent~=lp.Character and v.Parent:FindFirstChild("HumanoidRootPart") then local hrp=v.Parent.HumanoidRootPart; local bb=Instance.new("BillboardGui",hrp);bb.Name="__dfesp__";bb.Size=UDim2.new(0,0,0,26);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=v.Parent.Name;nl.TextColor3=Color3.fromRGB(255,80,80);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=11 end end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="A Universal Time Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
local flyBV,flyBG,flyOn=nil,nil,false; local function fly(on) flyOn=on; if flyBV then flyBV:Destroy();flyBV=nil end; if flyBG then flyBG:Destroy();flyBG=nil end; local hrp=getHRP();if not hrp then return end; if on then flyBV=Instance.new("BodyVelocity",hrp);flyBV.MaxForce=Vector3.new(1e5,1e5,1e5);flyBV.Velocity=Vector3.zero;flyBG=Instance.new("BodyGyro",hrp);flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5);flyBG.D=100; ac(rs.Heartbeat:Connect(function() if not flyOn or not flyBV then return end; local d=Vector3.zero; if uis:IsKeyDown(Enum.KeyCode.W) then d=d+cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.S) then d=d-cam.CFrame.LookVector end; if uis:IsKeyDown(Enum.KeyCode.A) then d=d-cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.D) then d=d+cam.CFrame.RightVector end; if uis:IsKeyDown(Enum.KeyCode.Space) then d=d+Vector3.new(0,1,0) end; if uis:IsKeyDown(Enum.KeyCode.LeftShift) then d=d-Vector3.new(0,1,0) end; flyBV.Velocity=d*100;flyBG.CFrame=cam.CFrame end)) end end
hub("A Universal Time Hub",{
    {"Speed+ (100 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 100 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Kill Aura (20st)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 and (r2.Position-hrp.Position).Magnitude<20 then pcall(function() v.Health=0 end) end end end end)) end end},
    {"Auto Farm NPCs",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; local near,nd=nil,120; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 then local d=(r2.Position-hrp.Position).Magnitude; if d<nd then near,nd=r2,d end end end end; if near then hrp.CFrame=CFrame.new(hrp.Position:Lerp(near.Position+Vector3.new(0,2,0),0.15)) end end)) end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"ESP All",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__autesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Parent~=lp.Character and v.Parent:FindFirstChild("HumanoidRootPart") then local hrp=v.Parent.HumanoidRootPart; local bb=Instance.new("BillboardGui",hrp);bb.Name="__autesp__";bb.Size=UDim2.new(0,0,0,26);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=v.Parent.Name;nl.TextColor3=Color3.fromRGB(130,200,255);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=11 end end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="King Legacy Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("King Legacy Hub",{
    ...
    {"Fly (WASD+Space)",fly},
    {"Kill Aura (25st)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 and (r2.Position-hrp.Position).Magnitude<25 then pcall(function() v.Health=0 end) end end end end)) end end},
    {"Auto Farm (Chase NPC)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; local near,nd=nil,150; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 then local d=(r2.Position-hrp.Position).Magnitude; if d<nd then near,nd=r2,d end end end end; if near then hrp.CFrame=CFrame.new(hrp.Position:Lerp(near.Position+Vector3.new(0,2,0),0.15)) end end)) end end},
    {"Devil Fruit ESP",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__klesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("fruit") or v.Name:lower():find("devil")) then local bb=Instance.new("BillboardGui",v);bb.Name="__klesp__";bb.Size=UDim2.new(0,0,0,28);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text="FRUIT: "..v.Name;nl.TextColor3=Color3.fromRGB(255,230,50);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=12 end end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="Type Soul Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("Type Soul Hub",{
    ...
    {"Fly (WASD+Space)",fly},
    {"Kill Aura (20st)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 and (r2.Position-hrp.Position).Magnitude<20 then pcall(function() v.Health=0 end) end end end end)) end end},
    {"Auto Grind (Chase)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; local near,nd=nil,100; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 then local d=(r2.Position-hrp.Position).Magnitude; if d<nd then near,nd=r2,d end end end end; if near then hrp.CFrame=CFrame.new(hrp.Position:Lerp(near.Position+Vector3.new(0,2,0),0.14)) end end)) end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"ESP All",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__tsesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Parent~=lp.Character and v.Parent:FindFirstChild("HumanoidRootPart") then local hrp=v.Parent.HumanoidRootPart; local bb=Instance.new("BillboardGui",hrp);bb.Name="__tsesp__";bb.Size=UDim2.new(0,0,0,24);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=v.Parent.Name;nl.TextColor3=Color3.fromRGB(180,140,255);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=11 end end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="Anime Last Stand Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
local function getHRP() local c=lp.Character; return c and c:FindFirstChild("HumanoidRootPart") end
hub("Anime Last Stand Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Inf Jump",function(on) if on then ac(game:GetService("UserInputService").JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Auto Place Units",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and v.Name:lower():find("place") and v.CanTouch then if (v.Position-hrp.Position).Magnitude<5 then hrp.CFrame=CFrame.new(v.Position+Vector3.new(0,3,0)) end end end end)) end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"Auto Sell (TP Shop)",function() local hrp=getHRP(); if hrp then hrp.CFrame=CFrame.new(0,5,0) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
})
]]},
        {name="Piggy Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("Piggy Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Noclip (Escape Piggy)",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"ESP Items (Keys etc)",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__pgesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("key") or v.Name:lower():find("item") or v.Name:lower():find("tool") or v.Name:lower():find("exit")) then local bb=Instance.new("BillboardGui",v);bb.Name="__pgesp__";bb.Size=UDim2.new(0,0,0,24);bb.StudsOffset=Vector3.new(0,2,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=v.Name;nl.TextColor3=Color3.fromRGB(80,255,180);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=11 end end end},
    {"ESP Piggy (Killer)",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__pgkesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Model") and (v.Name:lower():find("piggy") or v.Name:lower():find("bot")) then local hrp=v:FindFirstChild("HumanoidRootPart"); if hrp then local bb=Instance.new("BillboardGui",hrp);bb.Name="__pgkesp__";bb.Size=UDim2.new(0,0,0,28);bb.StudsOffset=Vector3.new(0,4,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text="[PIGGY] "..v.Name;nl.TextColor3=Color3.fromRGB(255,80,80);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=13 end end end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="Pressure Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("Pressure Hub",{
    ...
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"ESP Entities",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__pressesp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Model") and v~=lp.Character then local hrp=v:FindFirstChild("HumanoidRootPart"); local hum=v:FindFirstChildOfClass("Humanoid"); if hrp and hum and hum.Health>0 then local bb=Instance.new("BillboardGui",hrp);bb.Name="__pressesp__";bb.Size=UDim2.new(0,0,0,28);bb.StudsOffset=Vector3.new(0,4,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=v.Name;nl.TextColor3=Color3.fromRGB(255,100,100);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=12 end end end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Fullbright",function(on) local L=game:GetService("Lighting"); L.Brightness=on and 2 or 1; for _,v in pairs(L:GetDescendants()) do if v:IsA("BlurEffect") or v:IsA("ColorCorrectionEffect") then v.Enabled=not on end end end},
})
]]},
        {name="Deepwoken Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("Deepwoken Hub",{
    ...
    {"Kill Aura (20st)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 and (r2.Position-hrp.Position).Magnitude<20 then pcall(function() v.Health=0 end) end end end end)) end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"ESP Players",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__dwesp__" then v:Destroy() end end; return end; for _,p in pairs(pl:GetPlayers()) do if p~=lp and p.Character then local hrp=p.Character:FindFirstChild("HumanoidRootPart"); if hrp then local bb=Instance.new("BillboardGui",hrp);bb.Name="__dwesp__";bb.Size=UDim2.new(0,0,0,28);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=p.Name;nl.TextColor3=Color3.fromRGB(100,200,255);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=12 end end end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
})
]]},
        {name="DeadRails Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("DeadRails Hub",{
    {"Speed+ (70 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 70 or 16 end end},
    {"Fly (WASD+Space)",fly},
    {"Auto Collect Loot",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("gold") or v.Name:lower():find("ammo") or v.Name:lower():find("loot") or v.Name:lower():find("coin")) and v.CanTouch then if (v.Position-hrp.Position).Magnitude<3 then hrp.CFrame=CFrame.new(v.Position+Vector3.new(0,3,0)) elseif (v.Position-hrp.Position).Magnitude<70 then hrp.CFrame=CFrame.new(hrp.Position:Lerp(v.Position,0.1)) end end end end)) end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
    {"ESP Loot",function(on) if not on then for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BillboardGui") and v.Name=="__dresp__" then v:Destroy() end end; return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("gold") or v.Name:lower():find("ammo") or v.Name:lower():find("coin")) then local bb=Instance.new("BillboardGui",v);bb.Name="__dresp__";bb.Size=UDim2.new(0,0,0,22);bb.StudsOffset=Vector3.new(0,2,0);bb.AlwaysOnTop=true;local nl=Instance.new("TextLabel",bb);nl.Size=UDim2.new(1,0,1,0);nl.BackgroundTransparency=1;nl.Text=v.Name;nl.TextColor3=Color3.fromRGB(255,220,50);nl.TextStrokeTransparency=0;nl.Font=Enum.Font.GothamBold;nl.TextSize=11 end end end},
    {"Kill Aura (Undead)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("Humanoid") and v.Health>0 and v.Parent~=lp.Character then local r2=v.Parent:FindFirstChild("HumanoidRootPart"); if r2 and (r2.Position-hrp.Position).Magnitude<18 then pcall(function() v.Health=0 end) end end end end)) end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
})
]]},
        {name="Blade Ball Hub", builtIn=true, code=[[
local lp,pl,rs,uis,cam=game.Players.LocalPlayer,game:GetService("Players"),game:GetService("RunService"),game:GetService("UserInputService"),workspace.CurrentCamera
local conns={}; local function ac(c) table.insert(conns,c) return c end
local function hub(t,fs) local sg=lp:WaitForChild("PlayerGui"); local o=sg:FindFirstChild("__ch__"); if o then o:Destroy() end; local sc=Instance.new("ScreenGui",sg); sc.Name="__ch__"; sc.ResetOnSpawn=false; sc.IgnoreGuiInset=true; local w=Instance.new("Frame",sc); w.Size=UDim2.new(0,224,0,44+#fs*30); w.Position=UDim2.new(0.5,-112,0.22,0); w.BackgroundColor3=Color3.fromRGB(24,20,16); w.BorderSizePixel=0; Instance.new("UICorner",w).CornerRadius=UDim.new(0,12); local tl=Instance.new("TextLabel",w); tl.Size=UDim2.new(1,-36,0,36); tl.Position=UDim2.new(0,12,0,0); tl.BackgroundTransparency=1; tl.Text=t; tl.TextColor3=Color3.fromRGB(245,240,228); tl.Font=Enum.Font.GothamBold; tl.TextSize=14; tl.TextXAlignment=Enum.TextXAlignment.Left; local xb=Instance.new("TextButton",w); xb.Size=UDim2.new(0,26,0,26); xb.Position=UDim2.new(1,-30,0,5); xb.BackgroundColor3=Color3.fromRGB(235,75,75); xb.Text="X"; xb.TextColor3=Color3.new(1,1,1); xb.Font=Enum.Font.GothamBold; xb.TextSize=11; xb.BorderSizePixel=0; Instance.new("UICorner",xb).CornerRadius=UDim.new(0,6); xb.MouseButton1Click:Connect(function() for _,c in ipairs(conns) do pcall(function()c:Disconnect()end) end; sc:Destroy() end); local st={}; for i,e in ipairs(fs) do local b=Instance.new("TextButton",w); b.Size=UDim2.new(1,-16,0,26); b.Position=UDim2.new(0,8,0,40+(i-1)*28); b.BackgroundColor3=Color3.fromRGB(46,38,30); b.Text=e[1]; b.TextColor3=Color3.fromRGB(210,198,180); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.BorderSizePixel=0; Instance.new("UICorner",b).CornerRadius=UDim.new(0,8); st[i]=false; local ci=i; b.MouseButton1Click:Connect(function() st[ci]=not st[ci]; b.BackgroundColor3=st[ci] and Color3.fromRGB(210,168,95) or Color3.fromRGB(46,38,30); b.TextColor3=st[ci] and Color3.new(1,1,1) or Color3.fromRGB(210,198,180); pcall(e[2],st[ci]) end) end; local dg,ds,dp=false; w.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dg=true;ds=i.Position;dp=w.Position end end); w.InputEnded:Connect(function() dg=false end); uis.InputChanged:Connect(function(i) if dg then local d=i.Position-ds; w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end end) end
local function getHum() local c=lp.Character; return c and c:FindFirstChildOfClass("Humanoid") end
...
hub("Blade Ball Hub",{
    {"Speed+ (60 WS)",function(on) local h=getHum();if h then h.WalkSpeed=on and 60 or 16 end end},
    {"Inf Jump",function(on) if on then ac(uis.JumpRequest:Connect(function() local h=getHum();if h then h:ChangeState(Enum.HumanoidStateType.Jumping)end end)) end end},
    {"Auto Deflect (Ball Track)",function(on) if on then ac(rs.Heartbeat:Connect(function() local hrp=getHRP(); if not hrp then return end; for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and (v.Name:lower():find("ball") or v.Name:lower():find("blade")) and v.CanTouch then local d=(v.Position-hrp.Position).Magnitude; if d<12 then local pg=lp:FindFirstChild("PlayerGui"); if pg then for _,g in pairs(pg:GetDescendants()) do if (g:IsA("TextButton") or g:IsA("ImageButton")) and g.Visible then pcall(function() game:GetService("VirtualUser"):Button1Down(g.AbsolutePosition+g.AbsoluteSize*0.5,cam.CFrame); task.wait(0.05); game:GetService("VirtualUser"):Button1Up(g.AbsolutePosition+g.AbsoluteSize*0.5,cam.CFrame) end) end end end end end end end)) end end},
    {"Aimbot (Track Ball)",function(on) if on then ac(rs.RenderStepped:Connect(function() for _,v in pairs(workspace:GetDescendants()) do if v:IsA("BasePart") and v.Name:lower():find("ball") then cam.CFrame=CFrame.lookAt(cam.CFrame.Position,v.Position) end end end)) end end},
    {"Noclip",function(on) if on then ac(rs.Stepped:Connect(function() local c=lp.Character; if c then for _,p in pairs(c:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end end)) end end},
    {"God Mode",function(on) if on then ac(rs.Heartbeat:Connect(function() local h=getHum();if h then h.Health=h.MaxHealth end end)) end end},
})
]]},
