--[[
    RequireHub — Type an asset ID, press Load, done.
...
local function corner(f, r)
    ...
    c.CornerRadius = UDim.new(0, r or 8); c.Parent = f
end
local function stroke(f, col, thick)
    local s = Instance.new("UIStroke")
    s.Color = col or C.BORDER; s.Thickness = thick or 1; s.Parent = f
end
...
local ORB = Frm{
    ...
    BackgroundTransparency=1, ZIndex=101, Parent=INTRO,
}
local function ring(sz, col, tr)
    local f = Frm{
        Size=UDim2.fromOffset(sz,sz), BackgroundColor3=col,
        BackgroundTransparency=tr, ZIndex=101, Parent=ORB,
    }; corner(f, sz/2); return f
end
local R1 = ring(100, C.ACC,  1)
local R2 = ring(72,  C.ACC2, 1)
local R3 = ring(44,  C.WHITE, 1)
local RG = Lbl{
    Text="R", Font=Enum.Font.GothamBlack, TextSize=22,
    TextColor3=C.ACC, TextXAlignment=Enum.TextXAlignment.Center,
    Size=UDim2.fromOffset(44,44),
    AnchorPoint=Vector2.new(0.5,0.5), Position=UDim2.fromScale(0.5,0.5),
    TextTransparency=1, ZIndex=103, Parent=ORB,
}
local IT = Lbl{
    Text="RequireHub", Font=Enum.Font.GothamBlack, TextSize=28,
    TextColor3=C.WHITE, TextXAlignment=Enum.TextXAlignment.Center,
    Size=UDim2.new(1,0,0,36), AnchorPoint=Vector2.new(0.5,0),
    Position=UDim2.new(0.5,0, 0.575,0), TextTransparency=1,
    ZIndex=101, Parent=INTRO,
}
local IS = Lbl{
    Text="load any module by asset id", Font=Enum.Font.Gotham, TextSize=13,
    TextColor3=C.MUTED, TextXAlignment=Enum.TextXAlignment.Center,
    Size=UDim2.new(1,0,0,20), AnchorPoint=Vector2.new(0.5,0),
    Position=UDim2.new(0.5,0, 0.655,0), TextTransparency=1,
    ZIndex=101, Parent=INTRO,
}
...
task.spawn(function()
    ...
    tw(R3,{BackgroundTransparency=1},ti(0.4)); tw(RG,{TextTransparency=1},ti(0.3))
...
task.spawn(function()
    ...
    tw(WIN,{Size=UDim2.fromOffset(360,230), BackgroundTransparency=0},ti(0.45,"Back"))
    task.wait(0.1); tw(WIN,{Size=UDim2.fromOffset(340,218)},ti(0.22))
end)

-- ─── Header ───────────────────────────────────────────────────────────────────
local HDR = Frm{
    Size=UDim2.new(1,0,0,40), BackgroundColor3=C.PANEL,
...
-- Pulsing logo orb
local LORB = Frm{
    Size=UDim2.fromOffset(26,26), AnchorPoint=Vector2.new(0,0.5),
    Position=UDim2.new(0,9,0.5,0), BackgroundColor3=C.ACC,
    BorderSizePixel=0, ZIndex=3, Parent=HDR,
}
corner(LORB, 13)
task.spawn(function()
...
Lbl{Text="R", Font=Enum.Font.GothamBlack, TextSize=14, TextColor3=C.WHITE,
    ...
    Size=UDim2.fromScale(1,1), ZIndex=4, Parent=LORB}

Lbl{Text="RequireHub", Font=Enum.Font.GothamBlack, TextSize=14,
    TextColor3=C.TEXT, Size=UDim2.new(0,140,1,0),
    Position=UDim2.new(0,42,0,0), ZIndex=3, Parent=HDR}

local HMIN = Btn{
    Text="—", Font=Enum.Font.GothamBlack, TextSize=14, TextColor3=C.MUTED,
    Size=UDim2.fromOffset(26,26), AnchorPoint=Vector2.new(1,0.5),
    Position=UDim2.new(1,-34,0.5,0), BackgroundColor3=C.CARD,
    BorderSizePixel=0, ZIndex=3, Parent=HDR,
}
corner(HMIN, 6); hov(HMIN, C.CARD, C.HOVER)

local HCLOSE = Btn{
    Text="×", Font=Enum.Font.GothamBlack, TextSize=18,
    TextColor3=Color3.fromRGB(240,65,88),
    Size=UDim2.fromOffset(26,26), AnchorPoint=Vector2.new(1,0.5),
    Position=UDim2.new(1,-4,0.5,0), BackgroundColor3=C.CARD,
    BorderSizePixel=0, ZIndex=3, Parent=HDR,
}
...
-- Label
Lbl{
    Text="Module Asset ID",
    Font=Enum.Font.GothamBold, TextSize=11, TextColor3=C.MUTED,
    Size=UDim2.new(1,-24,0,14), Position=UDim2.new(0,12,0,12),
...
corner(IWRAP, 8); stroke(IWRAP, C.BORDER, 1)

local INP = Instance.new("TextBox")
INP.PlaceholderText = "e.g.  12345678"
...
INP.Focused:Connect(function()  tw(IWRAP,{BackgroundColor3=C.HOVER})
    stroke(IWRAP, C.BLIGHT, 1) end)
INP.FocusLost:Connect(function() tw(IWRAP,{BackgroundColor3=C.CARD})
    stroke(IWRAP, C.BORDER, 1) end)

-- Load button
local LBTN = Btn{
    Text="Load Module",
    Font=Enum.Font.GothamBlack, TextSize=14, TextColor3=C.WHITE,
    Size=UDim2.new(1,-24,0,38), Position=UDim2.new(0,12,0,80),
    BackgroundColor3=C.ACC, BorderSizePixel=0, ZIndex=2, Parent=BODY,
}
corner(LBTN, 9); hov(LBTN, C.ACC, Color3.fromRGB(135,80,255))

-- Status
local STAT = Lbl{
    Text="", Font=Enum.Font.Gotham, TextSize=11,
    TextColor3=C.MUTED, TextXAlignment=Enum.TextXAlignment.Center,
    Size=UDim2.new(1,-24,0,16), Position=UDim2.new(0,12,0,126),
    ZIndex=2, Parent=BODY,
}

-- ─── Load logic ───────────────────────────────────────────────────────────────
local function doLoad(idStr)
    idStr = idStr:match("^%s*(.-)%s*$")  -- trim
    local assetId = tonumber(idStr)
    if not assetId or assetId <= 0 then
        STAT.Text = "✗  Enter a valid numeric asset ID"
        STAT.TextColor3 = C.RED
        tw(IWRAP,{BackgroundColor3=Color3.fromRGB(40,12,16)})
        task.wait(0.15); tw(IWRAP,{BackgroundColor3=C.CARD})
        return
    end

    LBTN.Text = "Loading..."
    LBTN.BackgroundColor3 = C.YELLOW
    STAT.Text = "require("..assetId..")  ···"
    STAT.TextColor3 = C.MUTED
    ...
    if ok2 then
        LBTN.Text = "✓  Loaded"
        LBTN.BackgroundColor3 = C.GREEN
        STAT.Text = "require("..assetId..")  ✓"
        STAT.TextColor3 = C.GREEN
    else
        LBTN.Text = "✗  Failed"
        LBTN.BackgroundColor3 = C.RED
        STAT.Text = tostring(e2):sub(1, 60)
        STAT.TextColor3 = C.RED
        warn("[RequireHub] require("..assetId.."): "..tostring(e2))
    end
    task.wait(2.5)
    LBTN.Text = "Load Module"
    LBTN.BackgroundColor3 = C.ACC
end

LBTN.MouseButton1Click:Connect(function() doLoad(INP.Text) end)
INP.FocusLost:Connect(function(enter) if enter then doLoad(INP.Text) end end)
...
HMIN.MouseButton1Click:Connect(function()
    mini = not mini
    if mini then
        tw(WIN,{Size=UDim2.fromOffset(340,40)},ti(0.28)); HMIN.Text="+"
    else
        tw(WIN,{Size=UDim2.fromOffset(340,218)},ti(0.38,"Back")); HMIN.Text="—"
    end
end)
...
UIS.InputBegan:Connect(function(i, gp)
    ...
    if i.KeyCode==Enum.KeyCode.RightShift then
        ...
        if vis then
            WIN.Visible=true
            tw(WIN,{Size=UDim2.fromOffset(340,218),BackgroundTransparency=0},ti(0.32,"Back"))
        else
