#!/usr/bin/env python3
# ============================================================
#  run_urls.py  --  Luraph URL/webhook-capture harness (real Luau)
#
#  A focused variant of run.py aimed at one question: does the payload
#  contain any hardcoded URL / webhook / domain? It runs the sample under
#  a REAL Luau runtime in a network-blocked, stubbed environment and:
#    * hooks buffer.readstring UNFILTERED, so it logs the COMPLETE VM
#      constant pool (every string the v14.7 deserialiser materialises),
#      with a per-read counter to prove the pool was fully read;
#    * sniffs URL/discord/webhook/domain shapes from readstring AND from
#      runtime string assembly (string.char/sub/format/gsub, table.concat),
#      catching URLs built on the fly, not just stored whole;
#    * presets a fake valid getgenv().SCRIPT_KEY + a Jnkie SDK stub so a
#      key-gated payload takes its real (network) code path;
#    * makes Enum deeply chainable so nested Enum.X.Y access doesn't crash.
#
#  Emits [[URL]] / [[DOM]] hits, [[RS:n]] pool entries, [[RS-COUNT]] marks.
#  Base behaviour (self-unpack + W5 anti-tamper handling) is inherited from
#  run.py; see that file's header for the mechanism notes below.
#
...
-- deep-chainable stub for Enum-like nested access (Enum.X.Y.Z)
local chain
chain = setmetatable({}, {__index=function() return chain end, __call=function() return chain end,
    __concat=function() return "" end, __tostring=function() return "Enum" end,
    __eq=function() return false end})
env.Enum = chain
-- Pretend the user is authenticated so the payload takes its real (network) path.
env.getgenv().SCRIPT_KEY = "TEST-KEY-0000"
env.getgenv().Junkie = { service="snowy", identifier="1027906", provider="Mixed",
    check_key=function() return true, "ok" end,
    get_link=function() return "https://getsnowy.xyz/pricing" end }
env.getgenv().Junkie.checkKey = env.getgenv().Junkie.check_key
-- mark when the program reaches steady-state (first wait / first http)
local _hg = env.game.HttpGet
__STRINGS__
log("harness-ready")
...
STRINGS_BLOCK = r'''
do  -- FULL constant-pool + URL capture. Every string constant the v14.7
    -- deserialiser materialises flows through buffer.readstring; we log ALL
    -- of them (unfiltered) with a counter, and separately flag URL-shaped
    -- strings anywhere (readstring, char, sub, format, concat, gsub).
    local nread = 0
    local seenurl = {}
    local function urlish(s)
        if type(s)~="string" then return end
        for m in s:gmatch("https?://[%w%.%-_/%?%%&=:@~#%+]+") do
            if not seenurl[m] then seenurl[m]=true; print("[[URL]] "..string.format("%q", m)) end
        end
        local w = s:match("discord%.[%w]+/[%w%-]+") or s:match("dsc%.gg/[%w]+")
        if w and not seenurl[w] then seenurl[w]=true; print("[[URL]] "..string.format("%q", w)) end
        if #s>=8 and s:find("%.%a%a") and (s:find("webhook") or s:find("api%.") or s:find("%.xyz")
             or s:find("%.gg") or s:find("%.dev") or s:find("%.io") or s:find("%.net")) then
            if not seenurl[s] then seenurl[s]=true; print("[[DOM]] "..string.format("%q", s)) end
        end
    end
    local rb = realG.buffer
    if rb then
        local bp = {} ; for k,v in pairs(rb) do bp[k]=v end
        if rb.readstring then bp.readstring=function(...)
            local s = rb.readstring(...); nread = nread + 1
            if type(s)=="string" then
                urlish(s)
                if nread <= 4000 and #s>=1 then print("[[RS:"..nread.."]] "..string.format("%q", s):sub(1,300)) end
                if nread % 2000 == 0 then print("[[RS-COUNT]] "..nread) end
            end
            return s
        end end
        if rb.tostring then bp.tostring=function(...) local s=rb.tostring(...); urlish(s); return s end end
        env.buffer = bp
    end
    local rs = realG.string
    local sp = {} ; for k,v in pairs(rs) do sp[k]=v end
    sp.char  =function(...) local s=rs.char(...);  urlish(s); return s end
    sp.sub   =function(...) local s=rs.sub(...);   urlish(s); return s end
    sp.format=function(...) local s=rs.format(...);urlish(s); return s end
    sp.gsub  =function(...) local s,n=rs.gsub(...);urlish(s); return s,n end
    env.string = sp
    local rt = realG.table
    local tp = {} ; for k,v in pairs(rt) do tp[k]=v end
    tp.concat=function(...) local s=rt.concat(...); urlish(s); return s end
    env.table = tp
end
'''
