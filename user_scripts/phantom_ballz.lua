-- ============================================================================
--  🐉  LEVIATHAN — Phantom Ball
--
--  Phantom Ball autoparry, rendered fully in the Leviathan (NeverZen) UI.
--  Self-contained: the UI is embedded below — no loadstring/HttpGet inside.
--  Upstream logic: ltseverydayyou/uuuuuuu/PhantomBallz.luau (GameId 4538598064).
-- ============================================================================

local NeverZen = (function()
    -- NeverZen expects a global cloneref; provide a passthrough fallback
    -- for executors that don't ship one, so the embedded UI never hard-errors.
    local cloneref = (type(cloneref) == "function") and cloneref or function(x) return x end
-- Leviathan (NeverZen) UI — embedded inline (no remote load).
            ...
            function TabSignal:AddSection(Config)
                ...
                function SectionSignal:AddSlider(config)
                    ...
                    local updateSIZE = function()
                        ...
                        local valueWidth = math.max(scale.X + 10, 35) -- value box, min width

                        -- Total row width. Before the frame is laid out AbsoluteSize
                        -- is 0, so fall back to a sane default and let the
                        -- AbsoluteSize signal below re-run this once it's real.
                        local total = Slider.AbsoluteSize.X
                        if total <= 0 then total = 220 end

                        -- Cap the label so a long title can NEVER eat the whole row:
                        -- reserve the value box, the gaps and a minimum track, then
                        -- give the label whatever's left (it truncates past that).
                        local GAPS = 15
                        local MIN_TRACK = 45
                        local maxLabel = math.max(total - valueWidth - GAPS - MIN_TRACK, 0)
                        local labelWidth = math.clamp(scalelab.X + 10, 0, maxLabel)

                        value.Size = UDim2.new(0, valueWidth, 0, 15)
                        Label.Size = UDim2.new(0, labelWidth, 0.649999976, 0)
                        con.Position = UDim2.new(1, -(valueWidth + 8), 0.5, 0)
                        con.Size = UDim2.new(1, -(labelWidth + valueWidth + GAPS), 0.200000003, 0)
                    end

                    updateSIZE()
                    -- Recompute once the frame actually has a size (and on any resize),
                    -- so the track is measured against the real column width.
                    NeverZen:Track(Slider:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateSIZE))
...
local function safeSvc(name, cr)
	...
	local fn = type(cr) == "function" and cr or (type(cloneref) == "function" and cloneref or nil)
	if fn then
		local okRef, ref = pcall(fn, svc)
...
local function safeCM(root, method, ...)
	local obj = root
...
-- Service resolver: use the built-in safe fallbacks directly (the upstream
-- script fetched a remote resolver via loadstring; we stay self-contained).
local __lt = { cs = safeSvc, cm = safeCM }

local function safeReq(mod)
	if typeof(mod) ~= "Instance" or not mod.Parent or not mod:IsA("ModuleScript") then
		return nil
	end
...
local function waitChild(parent, name, tm)
	if typeof(parent) ~= "Instance" then
		return nil
	end
	return parent:FindFirstChild(name) or parent:WaitForChild(name, tm or 10)
end

local function stepSig()
	local r = safeSvc("RunService")
	return r and (r.PostSimulation or r.Heartbeat)
end

local function spamSig()
	local r = safeSvc("RunService")
	return r and (r.PostSimulation or r.Heartbeat)
end
...
local Plrs = __lt.cs("Players", cref)
local Run = __lt.cs("RunService", cref)
local SS = __lt.cs("SoundService", cref)
local TS = __lt.cs("TweenService", cref)
local RSrv = __lt.cs("ReplicatedStorage", cref)
local UIS = __lt.cs("UserInputService", cref)
local SG = __lt.cs("StarterGui", cref)
local WS = workspace
...
local TSFolder = waitChild(RSrv, "TS", 10)
local Remotes = waitChild(RSrv, "Remotes", 10)
local GenRemotes = TSFolder and TSFolder:FindFirstChild("GeneratedNetworkRemotes")
local BallSyncRemote = Remotes and Remotes:FindFirstChild("BallSyncData")

local bmMod = TSFolder and safeReq(TSFolder:FindFirstChild("BindableManager"))
local BM = bmMod and bmMod.BindableManager or nil
local enumFolder = TSFolder and TSFolder:FindFirstChild("Enums")
local abMod = enumFolder and safeReq(enumFolder:FindFirstChild("AbilityID"))
local AbilityID = abMod and abMod.AbilityID or { Deflect = "Deflect" }
local abilFolder = TSFolder and TSFolder:FindFirstChild("Abilities")
local defMod = abilFolder and safeReq(abilFolder:FindFirstChild("DeflectAbility"))
local Def = defMod and defMod.DeflectAbility or {}
...
local function refreshRefs()
	...
	if not (TSFolder and TSFolder.Parent) then
		TSFolder = RSrv:FindFirstChild("TS")
	end
	if not (Remotes and Remotes.Parent) then
		Remotes = RSrv:FindFirstChild("Remotes")
	end
	if TSFolder and not (GenRemotes and GenRemotes.Parent) then
		GenRemotes = TSFolder:FindFirstChild("GeneratedNetworkRemotes")
	end
	if Remotes and not (BallSyncRemote and BallSyncRemote.Parent) then
		BallSyncRemote = Remotes:FindFirstChild("BallSyncData")
	end
end

local oAvail
if type(hookfunction) == "function" and type(Def.IsAvailable) == "function" then
	pcall(function()
...
if not oAvail and type(Def) == "table" and type(Def.IsAvailable) == "function" then
	pcall(function()
		Def.IsAvailable = function(self, ...)
			return true
		end
	end)
end
...
local function validRemote(remote)
	return typeof(remote) == "Instance" and remote.Parent ~= nil and remote:IsA("RemoteEvent")
end

local gameEnv
local gameG

local function getGameGlobal()
	if type(gameG) == "table" then
		return gameG, gameEnv
	end

	if type(getfenv) == "function" then
		local fns = {}
		if type(Def) == "table" then
			fns[#fns + 1] = Def.Deflect
			fns[#fns + 1] = Def.OnClientCreate
			fns[#fns + 1] = Def.OnServerCreate
			fns[#fns + 1] = Def.constructor
		end
		...
		for i = 1, #fns do
			...
			if type(fn) == "function" then
				...
				if ok and type(env) == "table" then
					...
					gameG = type(g) == "table" and g or env
					return gameG, gameEnv
				end
			end
		end
	...
	gameEnv = type(_G) == "table" and _G or {}
	gameG = gameEnv
	return gameG, gameEnv
end

local function getGameDid()
	local g, env = getGameGlobal()
	local did = type(g) == "table" and rawget(g, "did") or nil
	if validRemote(did) then
		return did
	end

	did = type(env) == "table" and rawget(env, "did") or nil
	if validRemote(did) then
		return did
	end
...
local function setGameDid(remote)
	local g, env = getGameGlobal()
	if type(g) == "table" then
		rawset(g, "did", remote)
	end
	if type(env) == "table" and env ~= g and rawget(env, "_G") == nil then
		rawset(env, "did", remote)
	end
end

local function cacheParryRemote(remote, mode, locked)
	...
	if locked ~= nil then
		parryRemoteLocked = locked and true or false
	end
	...
	return parryRemote, parryMode
end

local function getGeneratedRemote(id)
	refreshRefs()
	if not TSFolder then
		return nil
	end
	if not (GenRemotes and GenRemotes.Parent) then
		GenRemotes = TSFolder:FindFirstChild("GeneratedNetworkRemotes")
	end
	...
	local remote = GenRemotes:FindFirstChild("RE_" .. tostring(id))
...
local function resolveParryRemote()
	...
	if parryRemoteLocked then
		if validRemote(parryRemote) then
			return parryRemote, parryMode
		end
		return nil
	end
	...
	local globalDid = getGameDid()
	if validRemote(globalDid) then
		return cacheParryRemote(globalDid, "buffer")
	end

	local byName = Remotes and Remotes:FindFirstChild("pr_" .. tostring(lp.UserId))
	if validRemote(byName) then
		return cacheParryRemote(byName, "buffer")
	end

	local generated = getGeneratedRemote(1)
	if validRemote(generated) then
		return cacheParryRemote(generated, "event")
	end
...
local function ensureParryInitialized()
	...
	local remote, mode = resolveParryRemote()
	if not remote then
		return false
	end
...
local function probeNamedRemoteEvents()
	refreshRefs()
	if probingRemote or not Remotes then
		return nil
	end
	...
	for i = 1, #candidates do
		...
		if remote and remote.Parent then
			...
			if changed then
				cacheParryRemote(remote, "buffer", true)
				parryInitDone = true
...
local function getDeflectAbility()
	if not BM or not AbilityID or AbilityID.Deflect == nil then
		return nil
	end
...
local function fireParryRemote(remote, mode, flag)
	if not validRemote(remote) then
		return false
	end
	...
	local payload = makeDeflectBuffer(flag or 1)
	if not payload then
		return false
	end
...
local function sendParry()
	local remote, mode = resolveParryRemote()
	if remote and fireParryRemote(remote, mode, 1) then
		return
	end

	local ability = getDeflectAbility()
	if ability then
		local did = rawget(ability, "did")
		if validRemote(did) and fireParryRemote((cacheParryRemote(did, "buffer")), "buffer", 1) then
			return
		end
	end

	remote, mode = probeNamedRemoteEvents()
	if remote and fireParryRemote(remote, mode, 1) then
		return
	end

	if ability and type(ability.Deflect) == "function" and validRemote(getGameDid()) then
		pcall(function()
			ability:Deflect()
		end)
	end
...
task.defer(function()
	local found, mode = resolveParryRemote()
	if found then
		ensureParryInitialized()
		notify("Parry remote detected successfully: " .. tostring(mode), true)
		return
	end

	found, mode = probeNamedRemoteEvents()
	if found then
		notify("Parry remote detected successfully: " .. tostring(mode), true)
	else
		ensureParryInitialized()
...
local gi
do
	local mod = TSFolder and safeReq(TSFolder:FindFirstChild("GameInfo"))
	if mod and mod.GameInfo then
		gi = mod.GameInfo
	end
end
...
local br
do
	local bf = TSFolder and TSFolder:FindFirstChild("BallReplicator")
	local mod = bf and safeReq(bf:FindFirstChild("BallReplicator"))
	if mod and mod.BallReplicator then
...
-- ── Leviathan config persistence ─────────────────────────────────────────────
-- Auto-save Spam / Visualizer / FX Clean and restore them next run. Declared at
-- file scope so updIcons() (the single choke point every state change funnels
-- through) can mark state dirty; a light 0.5s loop does the write. Every
-- filesystem call is existence-checked, so an executor without writefile just
-- runs without persistence instead of erroring.
local __lev_dirty = false
local __lev_saved = {}
do
	local HttpService = safeSvc("HttpService")
	if type(readfile) == "function" and type(isfile) == "function" and HttpService then
		pcall(function()
			if isfile("Leviathan/Phantom_config.json") then
				local d = HttpService:JSONDecode(readfile("Leviathan/Phantom_config.json"))
				if type(d) == "table" then __lev_saved = d end
			end
		end)
	end
		...
		task.spawn(function()
			while not dead do
				...
				if __lev_dirty then
					__lev_dirty = false
					pcall(function()
						if type(makefolder) == "function" and type(isfolder) == "function" and not isfolder("Leviathan") then
							makefolder("Leviathan")
						end
						writefile("Leviathan/Phantom_config.json", HttpService:JSONEncode({ spam = cfg.spam, vis = cfg.vis, fx = cfg.fx }))
					end)
				end
			end
		end)
...
local function runSpam(dt)
	...
	if spamLast > 0 then
		add = now - spamLast
	elseif type(dt) == "number" and dt > 0 then
		add = dt
	end
	spamLast = now

	if type(dt) == "number" and dt > add and dt < 0.25 then
		add = dt
	end

	if add < 0 then
		add = 0
	elseif add > spamStep * spamBurst then
		add = spamStep * spamBurst
	end

	spamAcc = math.min(spamAcc + add, spamStep * spamBurst)
	local sent = 0
	while spamAcc >= spamStep and sent < spamBurst do
		spamAcc -= spamStep
		sent += 1
		sendParry()
	end
...
local function setSpam(v)
	...
	if cfg.spam then
		...
		if not spamConn then
			local sig = spamSig()
			if sig then
				spamConn = sig:Connect(function(dt)
					runSpam(dt)
				end)
			end
		end
...
local function bind(ch)
	...
	simConn = sig:Connect(function(dt)
...
-- ── Leviathan UI (replaces the TopbarPlus Icon topbar menu) ──────────────────
-- Renders Phantom's controls in the shared Leviathan (NeverZen) window instead
-- of the TopbarPlus icon dropdown. No loadstring/HttpGet: the UI is embedded at
-- the top of this file. The mob.* adapters implement the exact API updIcons()
-- calls (:setLabel / :select / :deselect), so the parry logic is untouched.
do
	local ok, err = pcall(function()
		...
		local win = NeverZen.new({
			Name = "Leviathan",
			SubTitle = "Phantom Ball",
			Keybind = Enum.KeyCode.RightShift,
			Scale = UDim2.new(0, 480, 0, 300),
			Resizable = true,
			Shadow = false,
			Acrylic = false,
			ShowProfile = false,
		})
		local tab = win:AddTab({ Name = "Auto Parry" })
		...
		-- Adapter speaking the exact API updIcons() calls, backed by a Leviathan
		-- toggle. The per-control _sync guard stops a sync-driven SetValue from
		-- bouncing back into the feature setter (which would recurse via updIcons).
		local function makeControl(key, title, initial, onUser)
			local ctrl = { _sync = false }
			ctrl.handle = sec:AddToggle({
				Name = title,
				Default = initial and true or false,
				Callback = function(v)
					if ctrl._sync then return end
					onUser(v)
				end,
			})
			function ctrl:setLabel(t) labelText[key] = tostring(t); refreshStatus() end
			function ctrl:setState(v)
				self._sync = true
				pcall(function() self.handle:SetValue(v and true or false) end)
				self._sync = false
			end
			function ctrl:select() self:setState(true) end
			function ctrl:deselect() self:setState(false) end
			return ctrl
		end

		-- Restored value (saved config) or the script's default.
		local function want(key, dflt)
			local s = __lev_saved[key]
			if type(s) == "boolean" then return s end
			return dflt
		end
		local wSpam, wVis, wFx = want("spam", cfg.spam), want("vis", cfg.vis), want("fx", cfg.fx)
		...
		-- Drive the actual features to the restored state (also syncs visuals).
		setSpam(wSpam)
		setVis(wVis)
		setFx(wFx)
		updIcons()
		...
		pcall(function() Notifier.new("Phantom Ball", "Loaded in the Leviathan UI", 4) end)
	end)
	if not ok then
		warn("[Leviathan] Phantom UI failed to build: " .. tostring(err))
	end
end
