...
local function functionaliseSearchBar(instance, section)
	...
	instance:GetPropertyChangedSignal("Text"):Connect(function()
		if instance.Text == "" then
			...
			for i,v in pairs(section:GetChildren()) do
				if v:IsA("Frame") and string.find(v.Frame.GameName.Text:lower(), instance.Text:lower(), 1, true) then
					v.Visible = true;
...
EzHub.LocalLibFrame.AnimFrame1.ADD.Frame.Trigger.MouseButton1Click:Connect(function()
	...
	if EzHub.NavFrame.Position == UDim2.new(0, 0, 0, 34) then handleNavBtn(); end
...
local function getSuggestedCommand()
	for _, commandData in pairs(commands) do
		...
		for _, alias in pairs(aliases) do
			...
			if string.find(alias, EzHub.TerminalFrame.ExecuteFrame.ExecuteTextBox.Text, 1, true) == 1 then
				return alias;
			end
		end
	end
...
local function getSuggestedString()
	for i,v in pairs(customIntellisenseList) do
		if string.find(v, EzHub.TerminalFrame.ExecuteFrame.ExecuteTextBox.Text, 1, true) == 1 then
			return v;
		end
	end
end
