-- EzLib - Roblox UI library (bug-fixed & modernized)
--
-- Based on the original Ez Hub EzLib module. This version fixes a large number
-- of latent bugs and modernizes the code without changing the public API:
--   * Removed accidental global variable leaks (dragify update).
--   * Fixed CanvasSize auto-resize passing a UDim where a number was expected.
--   * Fixed slider default clamping/rounding and division-by-zero when min == max.
--   * Fixed ScrollBar colour maths (was operating on 0-1 floats as if 0-255).
--   * Fixed notification changeCallback writing to the wrong keys.
--   * Fixed button/slider/dropdown changeText targeting invalid instances/props.
--   * Fixed keybind element crashing on a nil default and setState not updating.
--   * Fixed create.changeKeybind calling :IsA on an Enum item.
--   * Keybind capture now ignores non-keyboard input.
--   * Cached services, replaced deprecated wait/spawn, added nil-callback guards.

local ezlib = {};

------------------------------------------------------------------------------
-- Cached services & compatibility helpers

local UserInputService = game:GetService("UserInputService");
local TweenService = game:GetService("TweenService");
local RunService = game:GetService("RunService");

-- task.* is available on modern engines/executors; fall back gracefully.
local taskWait = (task and task.wait) or wait;
local taskSpawn = (task and task.spawn) or function(fn, ...)
	return coroutine.wrap(fn)(...);
end

-- Preferred parent for protected GUIs (exploit environments expose gethui()).
...
-- Safe 0-1 ratio for slider positioning (avoids NaN when max == min).
local function sliderRatio(value, min, max)
...
UserInputService.InputBegan:Connect(function(input)
	-- Only accept real keyboard keys as a keybind (ignore mouse/touch/etc).
	if coreVars.awaitingInput and input.UserInputType == Enum.UserInputType.Keyboard then
...
if _G.EzHubTheme then
	...
	-- Only override the default theme if every colour was supplied correctly.
	if p and s and t and q then
		coreVars.colors = {
...
coreFuncs.rotateInstanceBy = function(instance, rotationAngle, delay, isCounterClockwise)
	for i = 1, rotationAngle / 40 do
		local tween = TweenService:Create(instance, TweenInfo.new(delay or 0.03, Enum.EasingStyle.Linear), {Rotation = instance.Rotation + (isCounterClockwise and -40 or 40)});
		tween:Play();
...
coreFuncs.applyFrameResizingLib = function(scrollingframe)
	...
	local function update()
		pcall(function()
			...
			-- CanvasSize expects numeric components; scroll vertically only.
			scrollingframe.CanvasSize = UDim2.new(0, 0, 0, cS.Y + 30);
		end)
	end
	calc:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(update);
	update();
end
...
coreGUIFuncs.newSlider = function(tabWindow, name, default, min, max, colors)
	...
	-- Clamp the default into range and round to a whole number.
	default = round(math.clamp(default or min, min, max));
	...
	local nameLabel = coreFuncs.addInstance("TextLabel", {
		["Name"] = "Value",
		["BackgroundColor3"] = Color3.fromRGB(255, 255, 255),
		["BackgroundTransparency"] = 1.000,
		["Position"] = UDim2.new(0, 15, 0, 0),
	...
	local ratio = sliderRatio(default, min, max);
	valueDisplay.Text = tostring(default);
	indicator.Position = UDim2.new(ratio, 0, 0.5, 0);
	indicatorTrail.Size = UDim2.new(ratio, 0, 1, 0);
	return ({["indicator"] = indicator, ["indicatortrail"] = indicatorTrail, ["valuedisplay"] = valueDisplay, ["sliderframe"] = sliderFrame, ["namelabel"] = nameLabel});
...
coreGUIFuncs.newKeybind = function(tabWindow, name, state, colors)
	...
	local button = coreFuncs.addInstance("TextButton", {
		...
		["AutoButtonColor"] = false,
		["Font"] = Enum.Font.SourceSans,
		["Text"] = (state and state.Name) or "None",
		["TextColor3"] = Color3.fromRGB(255, 255, 255),
		["TextSize"] = 14.000,
		["Parent"] = frame
	});
...
coreGUIFuncs.newDropdown = function(tabWindow, dropdownContainer, name, colors, mainGUI)
	...
	local uiNameLabel = coreFuncs.addInstance("TextLabel", {
		["Name"] = "UIName",
		["BackgroundTransparency"] = 1,
		["TextColor3"] = Color3.fromRGB(255, 255, 255),
		["AnchorPoint"] = Vector2.new(0.5, 0.5),
	...
	local mainScrollingFrame = coreFuncs.addInstance("ScrollingFrame", {
		...
		["ScrollBarThickness"] = 0,
		-- Color3 components are 0-1 floats; convert to 0-255, nudge, and clamp.
		["ScrollBarImageColor3"] = Color3.fromRGB(
			math.clamp(colors.Tertiary.R * 255 + 5, 0, 255),
			math.clamp(colors.Tertiary.G * 255 + 5, 0, 255),
			math.clamp(colors.Tertiary.B * 255 + 5, 0, 255)
		),
	...
	return ({["mainframe"] = mainFrame, ["mainscrollingframe"] = mainScrollingFrame, ["toggle"] = toggle, ["frame"] = frame, ["namelabel"] = nameLabel, ["uiname"] = uiNameLabel});
...
coreVars.notificationHolder = Instance.new("ScreenGui", guiParent);
ezlib.newNotif = function(notifType, text, buttonLT, buttonRT, buttonLC, buttonRC, theme)
	...
	notif.fireCallback = function(button)
		if button == ezlib.enum.button.left then
			if notif.callbackL then notif.callbackL() end
		elseif button == ezlib.enum.button.right then
			if notif.callbackR then notif.callbackR() end
		end
	end

	notif.changeCallback = function(button, callback)
		if button == ezlib.enum.button.left then
			notif.callbackL = callback;
		elseif button == ezlib.enum.button.right then
			notif.callbackR = callback;
		end
	end
	...
	notif.show = function()
		...
		repeat taskWait() until continueFunc;
		return notif;
	end
	...
	notif.playOnSeperateThread = function(delay)
		taskSpawn(function() notif.play(delay) end);
	end
...
ezlib.create = function(name, parent, pos, theme, gameID, deleteOldGUI)
	...
	local dropdownContainer = Instance.new("ScreenGui", guiParent);
	...
	UserInputService.InputBegan:Connect(function(input, gameProcessed)
		if gameProcessed then return end	-- don't toggle while the user is typing
	...
	create.newTab = function(name)
		...
		tab.newButton = function(name, callback)
			...
			button.changeText = function(text)
				buttonInstance.button.Text = text or "Button";
			end
		...
		tab.newSlider = function(name, state, min, max, callback)
			...
			-- Store the clamped/rounded value rather than the raw argument.
			slider.state = round(math.clamp(state or min, min, max));
			...
			UserInputService.InputChanged:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseMovement and coreVars.sliderUsed == sliderInstance then
					local scale = math.clamp((input.Position.X - sliderInstance.sliderframe.AbsolutePosition.X) / sliderInstance.sliderframe.AbsoluteSize.X, 0, 1);
					sliderInstance.indicator:TweenPosition(UDim2.new(scale, 0, 0.5, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true);
					sliderInstance.indicatortrail:TweenSize(UDim2.new(scale, 0, 1, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true);
					slider.state = round(scale * (max - min) + min);
			...
			slider.fireCallback = function(...)
				if slider.callback then slider.callback(...) end
			end

			slider.changeText = function(text)
				sliderInstance.namelabel.Text = text;
			end
			...
			slider.setState = function(state)
				state = round(math.clamp(state, min, max));
				slider.state = state;
				local scale = sliderRatio(state, min, max);
				sliderInstance.indicator.Position = UDim2.new(scale, 0, 0.5, 0);
				sliderInstance.indicatortrail.Size = UDim2.new(scale, 0, 1, 0);
				sliderInstance.valuedisplay.Text = tostring(state);
			end
		...
		tab.newCheckbox = function(name, state, callback)
			...
			checkbox.fireCallback = function(...)
				if checkbox.callback then checkbox.callback(...) end
			end
			...
			checkbox.setState = function(state)
				if state then
					TweenService:Create(checkboxInstance.toggle, TweenInfo.new(0.4), {BackgroundColor3 = Color3.fromRGB(0, 149, 74)}):Play();
				else
					TweenService:Create(checkboxInstance.toggle, TweenInfo.new(0.4), {BackgroundColor3 = Color3.fromRGB(149, 0, 0)}):Play();
				end
				checkbox.state = state;
			end
		...
		tab.newDropdown = function(name, state, data, callback, closeAfterButtonPress)
			...
			dropdownInstance.toggle.Text = tostring(dropdown.state or "");
			...
			local function handleDropdownTransparency(transparencyLevel)
				for i, v in pairs(dropdown.getMainInstance().mainframe:GetChildren()) do
					if v:IsA("GuiObject") then
						TweenService:Create(v, TweenInfo.new(0.25), {BackgroundTransparency = transparencyLevel}):Play();
						if pcall(function() local _ = v.Text end) then
							TweenService:Create(v, TweenInfo.new(0.25), {TextTransparency = transparencyLevel}):Play();
						end
					end
				end
			end
			...
			dropdown.showDropdown = function()
				...
				dropdown.movementConnection = RunService.RenderStepped:Connect(function()
				...
				repeat taskWait() until tweenDone;
			...
			dropdown.fireCallback = function(...)
				if dropdown.callback then dropdown.callback(...) end
			end

			dropdown.changeText = function(text)
				dropdownInstance.namelabel.Text = text;
			end
		...
		tab.newTextbox = function(name, state, callback)
			...
			textbox.fireCallback = function(...)
				if textbox.callback then textbox.callback(...) end
			end
		...
		tab.newKeybind = function(name, state, callback)
			...
			-- Pass the resolved (non-nil) keycode so the button renders correctly.
			local keybindInstance = coreGUIFuncs.newKeybind(tabInstance.window, name, keybind.state, theme);
			...
			keybindInstance.button.MouseButton1Click:Connect(function()
				...
				repeat taskWait() until not coreVars.awaitingInput;
	...
	create.changeKeybind = function(keyCode)
		if typeof(keyCode) == "EnumItem" and keyCode.EnumType == Enum.KeyCode then
			...
			error("changeKeybind expects an Enum.KeyCode");
		end
	end
