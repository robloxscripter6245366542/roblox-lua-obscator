-- ═══════════════════════════════════════════════════════════════════════
--  Preppy Hub  |  Spelling Bee  v2.3
--  Delta iOS/iPad + PC  —  Rayfield UI (native Preppy Hub fallback)
--  Auto-detects game · All remotes hooked · Genius-speed auto-submit
-- ═══════════════════════════════════════════════════════════════════════
...
-- ── Game detection (PlaceIds from PreppyHub game map) ─────────────────
local KNOWN_GAMES = {
...
-- UI update callbacks — overwritten by whichever GUI branch runs
local onWordDisplay = function(_w) end
local onStatsUpdate = function() end
...
-- Scans any arg depth (table / string / direct) for a word-like value
local function extractWord(args)
    for _, v in ipairs(args) do
        if looksLikeWord(v) then return v end
        if type(v) == "table" then
            for _, tv in pairs(v) do if looksLikeWord(tv) then return tv end end
        end
...
-- All game containers — every service that can hold remotes/values
...
-- Last-resort: fire every RemoteEvent in RS + WS when no named remote found
...
-- Genius-level static typing: 40 ms/char — ~250 WPM, no randomness
local CHAR_MS = 0.040
...
local submitAnswer  -- forward declared so onWordFound can reference it
...
-- ═══════════════════════════════════════════════════════════════════════
--  HOOKS
--  Patterns from repo's src/tabs/05_remotespy.lua + src/core/02_bridge.lua
-- ═══════════════════════════════════════════════════════════════════════
...
-- __namecall hook — uses `local _old; _old =` so the upvalue is always valid
-- (pattern from src/tabs/05_remotespy.lua)
        ...
        local _old; _old = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
            ...
            if m == "FireServer" or m == "InvokeServer" then
                ...
                if isRE or isRF then
                    ...
                    local learnedWord = extractWord({...})
                    if learnedWord and self:IsA("RemoteEvent") then answerRemote = self end
                    if isAnswerRemote(rname) and botEnabled and currentWord ~= "" then
                        if self:IsA("RemoteEvent") then
                            answerRemote = self; return _old(self, currentWord)
                        end
                    end
                end
            end
...
-- Live watcher: ChildAdded per-container + game.DescendantAdded for deep objects
-- (pattern from src/core/02_bridge.lua)
...
-- ═══════════════════════════════════════════════════════════════════════
--  INSTALL ALL HOOKS (before GUI so hooks are live immediately)
-- ═══════════════════════════════════════════════════════════════════════
...
-- Periodic visual scan — catches words displayed as TextLabels
task.spawn(function()
    while true do
        task.wait(0.5)
        if botEnabled then
...
-- ═══════════════════════════════════════════════════════════════════════
--  GUI — Try Rayfield (3-CDN chain), fall back to native Preppy Hub style
-- ═══════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════
--  BRANCH A — RAYFIELD UI
-- ══════════════════════════════════════════════════════════════════════
if Rayfield then

    local Window = Rayfield:CreateWindow({
        Name                 = "Preppy Hub  |  " .. GAME_NAME,
        LoadingTitle         = "Preppy Hub",
        LoadingSubtitle      = "Spelling Bee v2.3  •  " .. (isMobile and "iPad" or "PC"),
        Theme                = "Default",
    ...
    -- ── Bot Tab ───────────────────────────────────────────────────────
    local BotTab = Window:CreateTab("Bot")

    BotTab:CreateSection("Current Word")
    local WordLabel = BotTab:CreateLabel("Word: —")

    onWordDisplay = function(w)
        pcall(function() WordLabel:Set("Word: " .. w:upper()) end)
    end

    BotTab:CreateSection("Controls")
    ...
    BotTab:CreateSection("Actions")

    BotTab:CreateButton({
        Name     = "Submit Word Now",
        Callback = function()
            if currentWord == "" then
                Rayfield:Notify({Title="No Word", Content="No word captured yet.", Duration=3})
                return
            end
            submitAnswer(currentWord)
            Rayfield:Notify({Title="Submitted!", Content=currentWord:upper(), Duration=2})
        end,
    })
    ...
    BotTab:CreateButton({
        ...
        Callback = function()
            ...
            for _, root in ipairs(SCAN_ROOTS) do pcall(function() hookSVs(root) end) end
            Rayfield:Notify({Title="Re-scanned", Content="Hooks refreshed.", Duration=2})
        end,
    })
    ...
    local pWords  = StatsTab:CreateParagraph({Title="Words Typed", Content="0"})
    local pAvgWpm = StatsTab:CreateParagraph({Title="Avg WPM",     Content="0"})
    local pBest   = StatsTab:CreateParagraph({Title="Best WPM",    Content="0"})
    local pMist   = StatsTab:CreateParagraph({Title="Mistakes",    Content="0"})
    local pAcc    = StatsTab:CreateParagraph({Title="Accuracy",    Content="100%"})

    onStatsUpdate = function()
        local wpm = getWPM()
        local acc = wordsTyped == 0 and 100
                    or math.floor(((wordsTyped - mistakes) / wordsTyped) * 100)
        pcall(function() pWords:Set({Title="Words Typed", Content=tostring(wordsTyped)}) end)
        pcall(function() pAvgWpm:Set({Title="Avg WPM",    Content=tostring(wpm)}) end)
        pcall(function() pBest:Set({Title="Best WPM",     Content=tostring(bestWPM)}) end)
        pcall(function() pMist:Set({Title="Mistakes",     Content=tostring(mistakes)}) end)
        pcall(function() pAcc:Set({Title="Accuracy",      Content=acc .. "%"}) end)
    end
    ...
    StatsTab:CreateButton({
        ...
        Callback = function()
            ...
            Rayfield:Notify({Title="Stats Reset", Content="Session cleared.", Duration=2})
        end,
    })
    ...
    PlrTab:CreateSection("Player Options")
    ...
    InfoTab:CreateParagraph({
        Title   = "Game",
        Content = GAME_NAME .. (IS_KNOWN and "" or "  (universal mode)"),
    })
    InfoTab:CreateParagraph({
        Title   = "Hook Status",
        Content = _h2ok and "__namecall hook active  ✓" or "Event hooks only (no __namecall)",
    })
    InfoTab:CreateParagraph({
        Title   = "Typing Speed",
        Content = "~250 WPM  •  40 ms/char  •  static",
    })

    Rayfield:Notify({
        Title    = "Preppy Hub  |  " .. GAME_NAME,
        Content  = "Loaded ✓  •  Auto Type ON",
        Duration = 4,
    })

else
-- ══════════════════════════════════════════════════════════════════════
--  BRANCH B — NATIVE PREPPY HUB GUI  (Rayfield unavailable)
-- ══════════════════════════════════════════════════════════════════════

    local old2 = PGui:FindFirstChild("__PrepSpellHub__")
    ...
    verLbl.Text="Version 2.3  •  "..(isMobile and "iPad" or "PC")..(IS_KNOWN and "" or "  •  Universal")
    ...
    local _ll=Instance.new("UIListLayout",SC)
    _ll.Padding=UDim.new(0,6); _ll.SortOrder=Enum.SortOrder.LayoutOrder
    ...
    local function secHdr(txt)
        local f=Instance.new("Frame",SC); f.Size=UDim2.new(1,0,0,18)
    ...
    local wTitleL=Instance.new("TextLabel",wordCard)
    wTitleL.Size=UDim2.new(1,0,0,16); wTitleL.Position=UDim2.new(0,12,0,6)
    wTitleL.BackgroundTransparency=1; wTitleL.Text="CURRENT WORD"
    wTitleL.TextColor3=PINK; wTitleL.Font=Enum.Font.GothamBold
    wTitleL.TextSize=9; wTitleL.TextXAlignment=Enum.TextXAlignment.Left

    local nWord=Instance.new("TextLabel",wordCard)
    ...
    onWordDisplay = function(w) nWord.Text = w:upper() end
    ...
    local nWords = statCell("Words Typed", PINK)
    local nWpm   = statCell("Avg WPM",     PINK2)
    local nBest  = statCell("Best WPM",    Color3.fromRGB(255,200,60))
    local nMist  = statCell("Mistakes",    Color3.fromRGB(220,70,70))
    local nAcc   = statCell("Accuracy",    GREEN)
    ...
    Instance.new("Frame",SC).Size=UDim2.new(1,0,0,8)  -- bottom spacer
