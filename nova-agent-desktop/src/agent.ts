...
const SYSTEM =
  "You are Nova Agent, a coding assistant running as a desktop app over the user's chosen folder. " +
 ...
 * — so the app holds a connection to Claude only while it is in use.
 */
export class Agent {
  ...
  private async ensureClient(): Promise<Anthropic | null> {
    ...
    if (!apiKey) {
      this.events.onError("No API key set. Open Settings and paste your Anthropic API key.");
      return null;
    }
